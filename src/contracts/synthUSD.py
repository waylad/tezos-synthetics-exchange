...
def test():
    ...
    sUSD.mint(sp.record(address = elon.address,value = sp.nat(100))).run(sender = admin1)
    sUSD.mint(sp.record(address = mark.address,value = sp.nat(10))).run(sender = admin1)
    sUSD.burn(sp.record(address = elon.address,value = sp.nat(10))).run(sender = admin1)
    sUSD.transfer(sp.record(from_ = elon.address,to_ = mark.address,value = sp.nat(10))).run(sender=elon)
